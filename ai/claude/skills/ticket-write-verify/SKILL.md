...
| Code-fence mangling | Confluence | Markdown triple-backtick fenced code block (` ```lang `) submitted via `content_format: markdown` | Language tag glues onto the first content line with no newline (e.g. `yamlspaces:`), and underscores inside the block get escaped to `\_` — even though inline single-backtick code elsewhere on the same page is untouched. Fix: rewrite the section in `content_format: storage` using the native code macro (see [references/confluence-macros.md](references/confluence-macros.md) § Native code macro) |
