...
description: Run a layered functional health check on a service users report as broken, going past HTTP/blackbox status into response-body content, database health, a real auth-flow test, and background-job/worker signals. Use when a user says a service is "acting weird", "broken", "erroring", or "not working" despite monitoring showing green, when an HTTP 200 needs confirming as genuinely healthy rather than an error payload wrapped in a 200, or when asked to "check if X is actually working", "do a deeper health check", "the probe is green but users report errors", or "verify app health, not just uptime". Complements `infra-state-verify` (gates *deployed/live* claims) — this skill assumes deployment is already established and checks whether the service is *functionally healthy* underneath a green status. Each layer degrades gracefully without needed secret material — skip it explicitly rather than blocking the whole check.
