...
name: issue-update
description: Update a task or ticket — change status, add a comment, edit labels, close it, or sync the task index. Works across GitHub Issues, GitLab Issues, and Jira. Use when the user says "close issue #X", "mark PROJ-XXXXX done", "add a comment to #X", "update the priority on", or any similar task update request.
version: 1.0.0
...
Read `~/.config/claude-skills/local.json` when present ([references/local-config.md](../../../references/local-config.md)).
...
- `PROJECT-NNNNN` (work prefix) → Jira (`jira-adobe`)
...
Follow Jira conventions in `local.yaml` (`jira.*`).
