...
**When Argo CD automated sync/selfHeal is active on the target, check whether it needs to be suspended before a manual scale-to-0 or similar direct intervention.** selfHeal resetting a resource back to the declared replica count while you're mid-investigation looks like the intervention silently failing, and burns real investigation time chasing that instead of the actual incident — suspend automated sync first if the hypothesis test requires the resource to stay in a manually-set state.
