...
name: issue-create
description: Create a new task, issue, or story in the right system — GitHub Issues, GitLab Issues, or employer Jira — based on the current repo context. Handles template, routing, project assignment, issues log, and task index automatically. Use when the user asks to create a task, capture an action item, add something to the backlog, "log this as an issue", "make a ticket for", "create a story for", or similar. Context-aware: work org repos → Jira Story; GitLab repos → GitLab Issue; personal GitHub repos → GitHub Issue in that repo; no repo → Memex.
compatibility: Requires gh CLI, glab CLI, Atlassian MCP (for Jira).
version: 1.0.0
...
If `~/.config/claude-skills/local.yaml` exists, read it ([references/local-config.md](../../../references/local-config.md)). Use `jira.project_key`, `jira.base_url`, `jira.epic_link_field`, and `personal.github_user` for all steps below.
...
- `gitlab-current:<namespace/repo>` → Path D (GitLab Issue)
...
## Path A — Jira work Story

### A1. Gather information

- **Summary**: clear, imperative title
- **Component**: infer from context or leave blank if unclear
- **Epic link**: only if the user names a specific epic; use `${JIRA_EPIC_LINK_FIELD}` from local config
...
{
  "fields": {
    "project": { "key": "<JIRA_PROJECT_KEY from local.yaml>" },
    "summary": "<title>",
...
Capture the returned ticket key (e.g. `${JIRA_EXAMPLE_KEY}`).
...
Report: issue number and URL as a markdown link, project it was added to, and labels applied.

Example: "Created [#97 — Review HomeLab DNS config](https://github.com/...) → HomeLab project, priority/medium."
...
The `<namespace/repo>` comes from the `gitlab-current:` prefix output by `detect-context.sh` (e.g. `specterrealm/esports/minecraft/minecraft-modpack-cp-verdant`). Capture the returned URL and extract the issue number.

### D3. Append to task index
...
Report: issue number and URL as a markdown link.

Example: "Created [#12 — Fix spawn rates](https://gitlab.com/...) in specterrealm/esports/minecraft/minecraft-modpack-cp-verdant."
