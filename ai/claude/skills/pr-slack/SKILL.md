...
Read `~/.config/claude-skills/local.yaml` → `jira.base_url` ([references/local-config.md](../../../references/local-config.md)).
...
- **Branch name** — extract the Jira ticket key if present (e.g. `feat/PROJ-XXXXX-...` → `PROJ-XXXXX`)
...
`${JIRA_BASE_URL}/browse/{TICKET_KEY}` (from `local.yaml` → `jira.base_url`)
