...
Read `references/conventions.md` for the full naming rules and domain prefix table. Key points:

- Format: `{domain}-{verb}` (preferred) or `{domain}-{noun}`
- Lowercase letters and hyphens only, 1–64 characters, no leading/trailing/consecutive hyphens
- Must match the directory name exactly
- Pick the right domain prefix from the table in conventions.md (vault-, ces-, uv-, memex-, gh-, gl-, skill-, etc.)
