...
updated_by: claude
name: kb-entity-integrity-check
description: Scan a personal knowledge-base vault's entity data (person/org profile files) for missing canonical IDs, likely duplicate entities, and sensitivity-tag scope violations, surfacing findings for review rather than auto-fixing. Not HashiCorp Vault — this is the personal KB/notes vault (see vault-support for the secrets-management tool). Use after bulk entity edits, as a periodic KB-hygiene pass, or when asked "check the vault for duplicate entities", "did that migration create any orphan IDs", or "audit entity integrity". Read-only — never edits entity files itself.
compatibility: Works with any personal-KB vault storing entities as individual profile files (person/org markdown or similar) with frontmatter fields for ID and sensitivity scope.
---

# KB Entity Integrity Check
...
```
KB entity integrity — 3 findings
