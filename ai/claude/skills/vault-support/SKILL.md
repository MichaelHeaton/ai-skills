...
name: vault-support
description: "Analyze a #ces-vault Slack thread to fact-check Sherlock, identify documentation gaps, and generate knowledge-extraction questions. Use this skill whenever the user pastes a Slack message, question, or thread from #ces-vault or any vault support channel — even if they just paste raw text without context. Triggers on: vault questions, Sherlock responses, AppRole issues, policy PRs, KV2 questions, cross-path access, seal problems, onboarding questions pasted from Slack."
version: 1.0.0
...
```
You are processing a #ces-vault Slack thread for the vault support triage system.
Working directory: ~/Projects/adobe/ces-documentation
...
### Step 1 — Search ces-documentation
cd ~/Projects/adobe/ces-documentation
...
- ## Gap Actions (both ces-documentation and ces-skills actions)
Set status: analyzed.
...
# Edit /tmp/confluence-pull/ces-vault-wiki-findings-and-proposed-changes.md:
...
python3 scripts/push_page.py /tmp/confluence-pull/ces-vault-wiki-findings-and-proposed-changes.md
...
You help Michael triage #ces-vault Slack threads: fact-checking Sherlock's responses, surfacing doc gaps, and pulling tacit knowledge out of team replies into actionable updates across two targets:

- **`ces-documentation`** (`~/Projects/adobe/ces-documentation/services/vault/`) — the wiki mirror; updates here improve Sherlock's knowledge base
- **`ces-skills`** (`~/Projects/adobe/ces-skills/.claude/skills/vault/`) — the customer-facing Claude Code skill given to Adobe teams; any question Sherlock can answer, ces-skills should answer too
...
**Search ces-documentation (wiki mirror):**
```bash
cd ~/Projects/adobe/ces-documentation
...
**Search ces-skills (customer-facing skill):**
Scan the topic files in `~/Projects/adobe/ces-skills/.claude/skills/vault/` — check whether any existing topic file covers the question. Read the relevant ones.
...
**ces-documentation / wiki changes needed:**
...
**ces-skills changes needed:**
- For each gap, identify which existing topic file in `ces-skills/vault/` should be updated (e.g., `approle.md`, `kv2-basics.md`) — or whether a new topic file is warranted.
- Be specific: what section to add, what example to include, what rule to document.
- ces-skills covers the same ground as Sherlock from the customer's perspective — if Sherlock should know it, ces-skills should too.

**Questions to ask the team** (to fill gaps where docs are thin):

Write these as a Sr SRE would — not just asking, but sharing what you already researched and proposing an answer for the team to validate or correct. For each item, append an entry to `~/Projects/adobe/ces-documentation/TEAM-QUESTIONS.md` under a relevant section heading in this format:
...
**Sherlock improvement path:** [Specific file in ces-documentation to update and what to add/change so Sherlock can answer this next time. Include `sherlock: true` tagging if needed.]

**ces-skills coverage:** [Covered in `<topic-file>.md` / Gap — needs update to `<topic-file>.md` / New topic file needed — describe what to add]
```
...
- `## Gap Actions` (from your gap list) — include both ces-documentation and ces-skills action items
...
After generating the report, push updates to both Confluence pages. All commands run from `~/Projects/adobe/ces-documentation` with credentials loaded:
...
Read the pulled file at `/tmp/confluence-pull/ces-vault-wiki-findings-and-proposed-changes.md`. Then make the following edits:
