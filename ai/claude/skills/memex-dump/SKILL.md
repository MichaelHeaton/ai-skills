...
| Topic | Repo | System |
|---|---|---|
| Workstation setup, dotfiles, browser config (Brave/Chrome), dev environment, Homebrew, system tools, dashboard/homepage tools | `Michael-Heaton/workstation-devops` | GitLab |
...
|---|---|
| `jira-work` | Memex **unless** a named work epic was mentioned in this session |
| `github-current:<personal>` | That repo **if** the idea is clearly scoped to it; otherwise Memex |
| `gitlab-current:<namespace>` | That project **if** the idea is clearly scoped to it; otherwise Memex |
| `memex` | Memex |
...
- **Domain**: infer from context; ask only if completely ambiguous
  ...
  - Workstation-devops (GitLab): use `workstation` in the task index — no GitHub Project assignment applies
...
**Path GL — GitLab (workstation-devops or other scoped GitLab repo)**

The repo is already resolved by Step 1 routing — use `Michael-Heaton/workstation-devops` for workstation topics, or the `gitlab-current:` repo for other scoped ideas.

Seed labels (idempotent):

```bash
bash ~/.claude/skills/issue-create/scripts/seed-labels-gitlab.sh <resolved-repo>
```
...
glab issue create \
  --repo <resolved-repo> \
  --title "<title>" \
  --label "type/brain-dump,triage/needs-grooming,priority/<priority>" \
  --description "<lite user story body>"
```
...
**Path J — Jira (active work epic only)**

Create as Story with labels `brain-dump` and `needs-grooming`. Link to the active epic via `${JIRA_EPIC_LINK_FIELD}` from local config. See `issue-create` Path A for full Jira steps.
