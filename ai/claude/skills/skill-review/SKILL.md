...
- Internal ticket keys used as examples (real employer project keys)
...
Skills active this session: [comma-separated list]
...
1. Edit the skill's SKILL.md with the agreed changes
2. Read it back and confirm it looks right
3. Verify the skill has a row in `README.md` — add one if missing, update it if the name or purpose changed
4. Skim `AGENT.md` for any stale references to this skill (Key files table, Common tasks) and update them
5. Remind them to `git commit && git push` to sync to other workstations
