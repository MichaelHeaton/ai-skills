...
Before Steps 2–4, invoke the `git-ops` skill — it covers branching rules, commit format, PR format, and pre-commit checks. The short version: Adobe and GitLab repos always get a branch + PR; Memex is treated like Adobe. Full rules in `~/.claude/references/branching.md`.
...
Invoke `skill-review` in session-audit mode. It will reflect on the current conversation to find skill friction, missed triggers, and ungapped workflows worth turning into new skills. Findings always become tickets before being worked or deferred.
...
- If **yes** → invoke the `fewer-permission-prompts` skill
...
| Multiple unrelated tasks handled in one session | ⚠️ recommend fresh session next time |
...
Ask: "Were there any Jira tickets or GitHub Issues you worked on this session?"

If yes, for each ticket:
...
Use `jira_transition_issue` + `jira_add_comment` for Jira, `gh issue comment` + `gh issue edit` for GitHub.

**Why this matters**: Tickets that stay "In Progress" after a session ends give the next session (and teammates) a wrong picture of what's active.
