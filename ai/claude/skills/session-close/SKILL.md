...
**Multi-repo sessions: this check is provisional until the real target is known.** The `detect-context.sh` run above is scoped to whatever `$PWD` happened to be at session start — for a session that turns out to touch more than one repo, that's not necessarily where Step 10 will actually file the close-out ticket (see the multi-repo note immediately below: the real target is the repo with the most session activity, resolved only after Step 1). Don't treat a "no duplicate found" result from the `$PWD`-scoped search as final in that case. Once the multi-repo note below has resolved the actual target, re-run this same same-day search against that target before Step 10 files anything — only that re-run's result is authoritative for the duplicate check.
...
**Single-repo sessions hard-exclude `RECENT:n` sweep noise by default.** When the workspace has exactly one folder, the script already narrows the working set itself: `RECENT:n` repos found only by the broad `~/Projects/` sweep (not the workspace's own folder or the current repo) are left out of the output entirely — they're stale archive repos, not part of this session. This is a real exclusion, not just "skip the question" below. If the session is clearly single-repo-scoped from conversation context alone (no workspace file, or a multi-folder workspace where only one folder is actually in play), re-run with `SESSION_SINGLE_REPO=1` to get the same narrowing. **For a session scoped to more than one specific repo** (N>1) by conversation context alone, with no workspace file to infer it from, use `SESSION_SCOPED_REPOS="repoA,repoB"` (names under `~/Projects/`, or absolute paths) instead — same `RECENT:n` narrowing, generalized to the named set rather than only a single repo. The excluded repos are never lost — re-run with `SHOW_ALL_REPOS=1` to see them, which is what the "Show all repos" option below does. **This is exactly the mechanism the noisy-sweep question below resolves into** — a multi-folder workspace that is technically multi-repo but really had only one or a few repos actually in play this session is precisely the `SESSION_SCOPED_REPOS` case, not a genuine multi-repo session.

**Noisy sweep — ask a tighter question first.** When the discovery output has **more than 3 `RECENT:n` repos and 0–1 `RECENT:y` repos**, the full per-repo-option list below is mostly stale archive noise from the broad sweep, not real signal about this session. Before falling through to that list, ask the narrower question first:
> **Were you working in just one (or a named set of) repo(s) this session?**

- If the user names one repo or a small named set, set `SESSION_SCOPED_REPOS="repoA,repoB"` (or `SESSION_SINGLE_REPO=1` for exactly one) as described above, re-run Step 1's discovery with that scoping, and proceed with just those repos — skip the full list entirely.
- If the user says no, names repos outside the sweep, or wants to see everything, fall through to the full per-repo-option list below as normal.

This narrowing is scoped to the noisy-sweep shape above only — it does not replace the full-list fallback. A sweep with few `RECENT:n` repos, or one with multiple genuine `RECENT:y` repos, skips this tighter question and goes straight to the existing prompt below.
...
**A background/capture agent committing in a shared, non-worktree checkout can sweep unrelated files into someone else's open PR branch.** Before that agent's commit lands, verify its staged file set matches only what it was actually asked to touch — and if Step 9/10 later notices two PRs touching the same file merged close together, check for a squash-merge that raced a needed fix-up. Detection and recovery for both: [references/duplicate-content-race-check.md](references/duplicate-content-race-check.md).
