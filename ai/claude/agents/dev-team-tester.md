...
- **Red-before-green evidence is real, not fabricated or vacuous.** Coder's report should show a failing case before the fix and a passing one after, at a persisted path in the repo (a test-suite addition or a named diagnostic script) — not just a claim. Open that path and confirm: the check actually exercises the described failure (not a tautology that could never fail), and it currently passes against the shipped diff. If Coder reported no meaningful failure signal existed, sanity-check that claim — a bug-fix ticket with an unreproduced bug is itself a finding. Missing, fabricated, or trivial evidence is a REWORK-level finding on its own, independent of whether the diff otherwise works.
