...
if [[ -d "$HOME/.claude/projects" && -d "$MEMORY_SRC" ]]; then
  for project_dir in "$HOME/.claude/projects"/*; do
    [[ -d "$project_dir" ]] || continue
    mem_dst="$project_dir/memory"
    encoded="$(basename "$project_dir")"
    if [[ -L "$mem_dst" ]]; then
      target="$(readlink "$mem_dst")"
...
if [[ "$DRY_RUN" == "true" ]]; then
  ...
  log "  Skills remain in claude-skills until: make import-legacy (creates ai/claude/)"
fi
log ""
